...
internal static partial class CreateTextToVideoCommandApiCommand
{
    private static Option<bool?> IsPublic { get; } = CliRuntime.CreateNullableBoolOption(
        name: @"--is-public",
        description: @"Whether the generation is public on Haiper.");

    private static Option<bool?> IsEnablePromptEnhancer { get; } = CliRuntime.CreateNullableBoolOption(
        name: @"--is-enable-prompt-enhancer",
        description: @"Whether Haiper should enhance the prompt.");
    ...
    private static Option<global::Haiper.TextToVideoRequestGenMode?> GenMode { get; } = new(
        name: @"--gen-mode")
    {
        Description = @"",
    };
    ...
    public static Command Create()
    {
        var command = new Command(@"create-text-to-video", @"Create a Haiper 2.x text-to-video generation.");
                        ...
                        command.Options.Add(GenMode);                        command.Options.Add(SettingsOptions.Seed);
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            ...
            {
                        var __requestBase = await CliRuntime.ReadRequestOrDefaultAsync<global::Haiper.TextToVideoRequest>(
                            parseResult,
                            Input,
                            RequestJson,
                            RequestFile,
